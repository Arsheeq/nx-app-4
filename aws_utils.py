...
def get_cloudwatch_metric_data(cloudwatch, metric_name: str, namespace: str, 
    ...
    # Use 3 hour intervals for weekly reports, 30 min for daily
    period = 10800 if period_days > 1 else 1800  # 3 hours for weekly, 30 min for daily

    # Set shorter timeout for weekly reports to prevent worker timeout
    import socket
    original_timeout = socket.getdefaulttimeout()
    
    try:
        socket.setdefaulttimeout(10 if period_days > 1 else 30)
...
def get_instance_metrics(aws_access_key: str, aws_secret_key: str, 
    ...
    for resource in resource_list:
        try:
            ...
            if service_type == 'EC2':
                result = get_ec2_metrics(aws_access_key, aws_secret_key, instance_id, region, period_days)
                if result:
                    ...
                    if 'disk_metrics' in result:
                        for disk_name, disk_data in result['disk_metrics'].items():
                            if disk_data['Datapoints']:
                                processed_result['metrics'][disk_name] = process_metric_data(disk_data)
